...
vector<pair<char, int>> string_Processing(string& name)
{
    ...
    for(int i = 0; i < name.size(); i++){
        char check = name[i];
        vector<pair<char,int>>::iterator it = find_if(freq_prev.begin(),freq_prev.end(),
                                                      [check](const pair<char,int>&element){return element.first == check;});
        if(it != freq_prev.end()){
            it->second++;
        }
        else{
            freq_prev.push_back(pair<char,int>(name[i],1));
        }
    }
    ...
    for(int i = 0; i < freq_prev.size(); i++){
        char change = freq_prev[i].first;
        if(isupper(freq_prev[i].first)){
            freq_prev[i].first = char(int(freq_prev[i].first + freq_prev[i].second - 65) % 26 + 65);
        }
        else{
            freq_prev[i].first = char(int(freq_prev[i].first + freq_prev[i].second - 97) % 26 + 97);
        }
        for(int j = index; j < name.size(); j++){
            if(name[j] == change){
                name[j] = freq_prev[i].first;
            }
        }
        index++;
    }
    ...
    for(int i = 0; i < freq_prev.size(); i++){
        pair<char,int>checkPair = freq_prev[i];
        bool newPair = true;
        for(int j = 0; j < freq.size(); j++){
            if(checkPair.first == freq[j].first){
                freq[j].second += checkPair.second;
                newPair = false;
                break;
            }
        }
        if(newPair){
            freq.push_back(checkPair);
        }
    }
    ...
    sort(freq.begin(),freq.end(), check);
...
int main()
{
    ...
    name = "HelloWorld";
    cout << "test 9 - name = " << name << " : ";
    you = string_Processing(name);
    for(int i = 0; i < you.size(); i++){
        cout << "(" << you[i].first << "," << you[i].second << ")";
    }
    cout << "name_you  : " << name << endl;
}
